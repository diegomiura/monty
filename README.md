...
_No upcoming fixtures with both teams decided — regenerate after the current round._
...
Every row was written to the append-only [prediction log](reports/predictions/world_cup_2026_predictions.csv) **before kickoff**, then graded against the official result. Predictions are never rewritten — only the result columns are filled in. Generated **2026-07-26T20:00:13Z**.

**2/5 results correct** · **3/5 exact scores** · log loss **1.1037** (a uniform 33/33/33 guess scores 1.0986) · Brier **0.7159**

| Match | Mode | Model's pick | Actual (90 min) | Result |
|:--|:--|:--|:--|:--:|
| England v Argentina | rolling | Argentina 41% | Argentina 1–2 | ✅ |
| France v Spain | rolling | Spain 35% | Spain 0–2 | ✅ |
| Argentina v Switzerland | rolling | Argentina 57% | Draw 1–1 _(a.e.t.)_ | ❌ 🎯 |
| Argentina v Switzerland | frozen | Argentina 56% | Draw 1–1 _(a.e.t.)_ | ❌ 🎯 |
| Norway v England | rolling | England 49% | Draw 1–1 _(a.e.t.)_ | ❌ 🎯 |

✅ correct result · ❌ wrong · 🎯 exact scoreline predicted · _a.e.t._ / _pens_ = level after 90 minutes and settled later; graded on the 90-minute score, which is what the model forecasts.

A ❌ 🎯 row is not a contradiction: the most likely *single scoreline* can be 1–1 while the most likely *outcome* is still a win, because the win probability sums over many scorelines.

**These predictions scored worse than a uniform guess** (1.1037 vs 1.0986 log loss). 3 of the 3 misses finished level after 90 minutes while the model favoured one side (all settled in extra time or on penalties). Reported as-is.

1 logged prediction(s) not shown: the fixture was never played (a speculative matchup the bracket did not produce).

> **Sample size: 5 matches.** Far too small to judge calibration — one result swings every number in that summary. The meaningful evaluation is the 290-match World Cup backtest in [Results](#results); this table is a live, pre-registered honesty check, not a performance claim.
